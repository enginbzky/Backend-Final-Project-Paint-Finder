...
router.post("/paints", async (req, res, next) => {
  try {
    const { paintName, brand, material, description, maxSpeed } = req.body;
    const newPaint = { brand, paintName, material, description, maxSpeed };
...
router.delete("/:id", async (req, res, next) => {
  try {
    ...
    res.send(`paint with the id ${id} deleted`);
  } catch (err) {
    console.error(err);
...
router.put("/paint/:id", async (req, res, next) => {
  try {
    const changePaintInfo = await paintRepository.changePaintInfo(req.body);
    res.json(changePaintInfo);
  } catch (error) {
    console.log(error);
    next(error);
  }
